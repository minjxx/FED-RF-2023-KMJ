// 상품 전체 리스트 페이지
export function GList() {
  return (
    <main id="cont">
      <section>
        <div id="optbx">
          <label for="men">남성</label>
          <input type="checkbox" id="men" /> <label for="women">여성</label> <input type="checkbox" id="women" /> <label for="style">스타일</label> <input type="checkbox" id="style" />
        </div>{" "}
        <div className="grid">
          <div>
            <a href="#/detail" className="">
              [1]
              <img src="./images/goods/men/m1.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]카모전판프린트 PQ 티셔츠</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [2]
              <img src="./images/goods/style/m7.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]블럭형 풀집업 래쉬가드</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [3]
              <img src="./images/goods/men/m8.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]베이직 솔리드 래쉬가드</h2> <h3>49,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [4]
              <img src="./images/goods/style/m8.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]베이직 솔리드 래쉬가드</h2> <h3>49,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [5]
              <img src="./images/goods/women/m2.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]빅로고 컬러 블럭 PQ 티셔츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [6]
              <img src="./images/goods/men/m9.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]남성 루즈핏 슬리브리스</h2> <h3>59,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [7]
              <img src="./images/goods/women/m3.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]빅로고 컬러 블럭 PQ 티셔츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [8]
              <img src="./images/goods/style/m1.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]카모전판프린트 PQ 티셔츠</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [9]
              <img src="./images/goods/women/m4.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]부분 스트라이프 PQ 티셔츠</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [10]
              <img src="./images/goods/style/m2.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]빅로고 컬러 블럭 PQ 티셔츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [11]
              <img src="./images/goods/style/m3.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]빅로고 컬러 블럭 PQ 티셔츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [12]
              <img src="./images/goods/women/m1.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]카모전판프린트 PQ 티셔츠</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [13]
              <img src="./images/goods/women/m5.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]웰딩포인트 트레이닝 하프팬츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [14]
              <img src="./images/goods/men/m2.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]빅로고 컬러 블럭 PQ 티셔츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [15]
              <img src="./images/goods/men/m3.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]빅로고 컬러 블럭 PQ 티셔츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [16]
              <img src="./images/goods/women/m6.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]블럭형 풀집업 래쉬가드</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [17]
              <img src="./images/goods/women/m9.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]여성 루즈핏 슬리브리스</h2> <h3>59,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [18]
              <img src="./images/goods/men/m4.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]부분 스트라이프 PQ 티셔츠</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [19]
              <img src="./images/goods/women/m7.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]블럭형 풀집업 래쉬가드</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [20]
              <img src="./images/goods/women/m8.png" alt="dress" />{" "}
              <aside>
                <h2>[여성]베이직 솔리드 래쉬가드</h2> <h3>49,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [21]
              <img src="./images/goods/men/m7.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]블럭형 풀집업 래쉬가드</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [22]
              <img src="./images/goods/style/m4.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]부분 스트라이프 PQ 티셔츠</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [23]
              <img src="./images/goods/men/m5.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]웰딩포인트 트레이닝 하프팬츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [24]
              <img src="./images/goods/style/m5.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]웰딩포인트 트레이닝 하프팬츠</h2> <h3>89,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [25]
              <img src="./images/goods/men/m6.png" alt="dress" />{" "}
              <aside>
                <h2>[남성]블럭형 풀집업 래쉬가드</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [26]
              <img src="./images/goods/style/m6.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]블럭형 풀집업 래쉬가드</h2> <h3>99,000원</h3>
              </aside>
            </a>
          </div>
          <div>
            <a href="#/detail" className="">
              [27]
              <img src="./images/goods/style/m9.png" alt="dress" />{" "}
              <aside>
                <h2>[스타일]루즈핏 슬리브리스</h2> <h3>59,000원</h3>
              </aside>
            </a>
          </div>
        </div>
      </section>
    </main>
  );
} /////////// GList 컴포넌트 /////////
